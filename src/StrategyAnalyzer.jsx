...
export default function StrategyAnalyzer() {
  ...
  // Alerts state
  const [alertStatuses, setAlertStatuses] = useState({}); // {symbol: scanResult}
  const [alertHistory, setAlertHistory] = useState([]);
  const handleTickerStatus = useCallback((symbol, result) => {
    setAlertStatuses(prev => ({ ...prev, [symbol]: result }));
  }, []);
  const handleAlert = useCallback((alert) => {
    setAlertHistory(prev => [...prev.slice(-49), alert]); // keep last 50
  }, []);
  ...
  useAlertScanner({
    ...
    intervalSec: settings.alertScanIntervalSec,
    useSound: settings.alertSound,
    useNotification: settings.alertNotification,
    onAlert: handleAlert,
    onTickerStatus: handleTickerStatus,
  });
    ...
    <div style={{ minHeight: '100vh', background: '#020617', color: '#e2e8f0', padding: '20px', fontFamily: fonts.sans }}>
      ...
      <div style={{ maxWidth: '1500px', margin: '0 auto' }}>
        ...
        <AlertsPanel
          enabled={settings.alertsEnabled}
          tickers={activeWatchlist?.tickers || []}
          scanInterval={settings.alertScanIntervalSec}
