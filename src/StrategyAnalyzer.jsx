...
export default function StrategyAnalyzer() {
  ...
  const derived = useMemo(() => {
    ...
    const currentPrice = override.currentPrice ?? meta?.regularMarketPrice ?? (intraday5m.length ? intraday5m[intraday5m.length - 1].c : null);
    // FIX: meta.previousClose is yesterday's actual close. meta.chartPreviousClose is the close at the START of the chart range (months ago).
    const prevClose = override.prevClose ?? meta?.previousClose ?? (daily.length > 1 ? daily[daily.length - 2].c : null);
      ...
      <div style={{ maxWidth: '1500px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', background: '#0f172a', border: '1px solid #1e293b', borderRadius: '4px', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
          ...
          {derived.currentPrice != null && (
            <div style={{ display: 'flex', gap: '20px', fontFamily: fonts.mono, fontSize: '12px' }}>
              ...
              <span style={{ color: '#64748b' }}>VEILLE <span style={{ color: '#f1f5f9' }}>{derived.prevClose?.toFixed(2)}</span></span>
